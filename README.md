...
Ver o desenho conceitual dessa fronteira em [documentacao-base-local/04-specs.md §8](./documentacao-base-local/04-specs.md#8-a-ponte) e nos componentes "Tradutor da Ponte" / "Store de Tasks" de [documentacao-base-local/02-architecture.md §3.2](./documentacao-base-local/02-architecture.md#32-componentes-do-agente).

## Decisões técnicas

- **Proteção do `requestState`**: HMAC-SHA256 via `node:crypto`, chave de `REQUEST_STATE_SECRET` (≥32 bytes) — ver [documentacao-base-local/03-adr.md ADR-0003](./documentacao-base-local/03-adr.md#adr-0003--proteção-do-requeststate-com-hmac-sha256-stateless). TODO: confirmar biblioteca final e detalhar o payload assinado.
